...
static const char* ASSET_PLAYER_IDLE = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/sprites/player-idle-2.png";   // 4 frames animados
static const char* ASSET_PLAYER_WALK = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/sprites/player-walk-2.png";   // ~10 frames
...
void player_init(Player* p, double x0) {
 ...
 void game_load_resources(Game* game) {
    ...
    if (!game->player.sprite_walk) {
        ...
        // player-walk-2.png: primeiro frame está cortado, pular frame 0
        game->player.walk_frame_offset = 1;  // Pula primeiro frame cortado
        
        // Walk: detectar frames automaticamente
        if (w > h) {
            // Spritesheet horizontal: dividir largura pela altura
            int total_frames = w / h;
            // Usar 8 frames úteis (pula o primeiro, usa frames 1-8)
            game->player.frames_walk = 8;
            printf("[INFO] Walk sprite: %dx%d, total %d frames, usando frames 1-8 (pulando frame 0 cortado)\n", 
                   w, h, total_frames);
        }
        else {
            // Fallback
