 ...
 */
 ...
 void player_draw(const Player* p, int screen_x, int screen_y) {
     ...
     // Debug: verificar se sprites estão carregados
     printf("[DEBUG] Player draw - State: %d, Idle: %p, Walk: %p, Shoot: %p\n",
         p->state, p->sprite_idle, p->sprite_walk, p->sprite_shoot);
     ...
     if (current_sprite) {
         ...
         printf("[DEBUG] Sprite carregado - W: %d, H: %d, Frame: %d\n", bmp_w, bmp_h, p->frame);
 
         // Shoot: verificar layout (1x6 ou 2x4)
         if (p->state == PLAYER_SHOOT && bmp_w == 384 && bmp_h == 64) {
             // Layout 1x6 (384x64)
             int sx = p->frame * frame_width;
             al_draw_bitmap_region(current_sprite, sx, 0, frame_width, frame_height,
                 screen_x - frame_width / 2, screen_y - frame_height, flags);
         }
         else if (p->state == PLAYER_SHOOT) {
             // Layout 2x4 ou outro grid
             draw_grid_frame(current_sprite, p->frame, p->shoot_cols, p->shoot_rows,
                 screen_x - frame_width / 2, screen_y - frame_height,
                 frame_width, frame_height, flags);
         }
         else {
             // Idle e Walk: 1xN (256x64) - CORRIGIDO: usar frame_height/2 para centralizar
             int sx = p->frame * frame_width;
             al_draw_bitmap_region(current_sprite, sx, 0, frame_width, frame_height,
                 screen_x - frame_width / 2, screen_y - frame_height, flags);
         }
         ...
         printf("[DEBUG] Usando fallback - boneco palito\n");
 ...
 void arrow_update(Arrow* arrow, double dt, const MathFunction* fn) {
     ...
     arrow->current_y = fn_eval(fn, arrow->current_x);
 ...
 void level_init_chapters(Chapter* chapters) {
     ...
     // TEST
     chapters[0].test.arrows_allowed = 3;
     chapters[0].test.epsilon_px = 16.0;
     chapters[0].test.color_tint = TINT_BRONZE;
     chapters[0].test.collectible_count = 2;
     chapters[0].test.items[0] = (Collectible){ 6.0, 2.0, 0.40, COLLECT_KEY, false };
     chapters[0].test.items[1] = (Collectible){ 5.0, 1.0, 0.35, COLLECT_APPLE, false };
     ...
     for (int i = 1; i < 6; i++) {
         ...
         switch (i) {
         case 1: chapters[i].name = "Quadratica"; chapters[i].theme_color = al_map_rgb(150, 50, 200); break;
         case 2: chapters[i].name = "Exponencial"; chapters[i].theme_color = al_map_rgb(255, 180, 0); break;
         case 3: chapters[i].name = "Logaritmica"; chapters[i].theme_color = al_map_rgb(0, 200, 100); break;
         case 4: chapters[i].name = "Racional"; chapters[i].theme_color = al_map_rgb(200, 100, 50); break;
         case 5: chapters[i].name = "Trigonometrica"; chapters[i].theme_color = al_map_rgb(255, 100, 150); break;
         }
     }
 }
 ...
 void game_load_resources(Game* game) {
     ...
     if (!game->player.sprite_idle) {
         ...
         printf("[SUCCESS] Sprite idle carregado!\n");
     }
     ...
     if (!game->player.sprite_walk) {
         ...
         printf("[SUCCESS] Sprite walk carregado!\n");
     }
     ...
     case STATE_PLAY:
         ...
         if (space_released_now) {
             ...
             if (!game->arrow.active && game->arrows_remaining > 0) {
                 ...
                 arrow_shoot(&game->arrow, game->player.x, game->player.y, vx);
 ...
 void game_draw(Game* game) {
     ...
     switch (game->state) {
     case STATE_TITLE:
         al_draw_text(font, al_map_rgb(255, 255, 100), SCREEN_WIDTH / 2, 150,
         ...
         al_draw_text(font, al_map_rgb(150, 255, 150), SCREEN_WIDTH / 2, 400,
             ALLEGRO_ALIGN_CENTER, "ENTER - Jogar");
         al_draw_text(font, al_map_rgb(255, 150, 150), SCREEN_WIDTH / 2, 430,
     ...
     case STATE_CHAPTER_SELECT:
         ...
         for (int i = 0; i < 6; i++) {
             ALLEGRO_COLOR color = (i == game->chapter_menu_selection) ?
                 al_map_rgb(255, 255, 100) : al_map_rgb(180, 180, 180);
             char text[64];
         ...
         al_draw_text(font, al_map_rgb(150, 150, 150), SCREEN_WIDTH / 2, 500,
             ALLEGRO_ALIGN_CENTER, "Use 1-6 para selecionar");
         break;
     ...
     case STATE_PLAY: {
         ...
         if (game->ui.preview_enabled || game->key_shift) {
             ...
             arrow_draw_preview(game->player.x, game->player.y, vx_preview,
         ...
         al_draw_text(font, al_map_rgb(180, 180, 180), SCREEN_WIDTH / 2, SCREEN_HEIGHT - 30,
             ALLEGRO_ALIGN_CENTER, "[<-/->]=mover [ESPACO]=segurar/soltar [M]=m [K]=k [Q]=diminuir [E]=aumentar [H]=especial");
         break;
     }
