...
static const char* ASSET_PLAYER_SHOOT = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/sprites/player-walk-3.png";
...
void init_function_params(MathFunction* fn) {
    ...
    case FN_LOGARITHMIC:
        ...
        break;
    case FN_RATIONAL:
        fn->params.rational.a = 2.0;  // AUMENTADO (era 1.0)
        fn->params.rational.b = 0.0;
        fn->params.rational.c = 0.5;  // REDUZIDO (era 1.0) - evita assíntotas próximas
        fn->params.rational.d = 2.0;  // AUMENTADO (era 1.0)
        break;
    case FN_TRIGONOMETRIC:
...
bool check_arrow_obstacle_collision(Arrow* arrow, Obstacle* obstacles, int count) {
    if (!arrow->active || !arrow->following_function) return false;
...
static TutorialScreen tutorial_linear[] = {
    {
        "FUNCAO LINEAR",
        {
            "Equacao: y = mx + k",
            "",
            "m: Inclinacao da reta (quanto sobe/desce)",
            "k: Intercepto (onde cruza o eixo Y)",
            "",
            "Exemplo: y = 2x + 1",
            "Se m=2: a cada 1 unidade em x, sobe 2 em y",
            "Se k=1: a reta passa pelo ponto (0, 1)",
            "",
...
static TutorialScreen tutorial_quadratic[] = {
    {
        "FUNCAO QUADRATICA",
        {
            ...
            "",
            "Exemplo: y = 0.5(x-2)^2 + 1",
            "Parabola com vertice em (2, 1) abrindo para cima",
            "",
            "Pressione ENTER para iniciar!"
        },
        10
...
static TutorialScreen tutorial_logarithmic[] = {
    {
        "FUNCAO LOGARITMICA",
        {
            ...
            "c: Deslocamento vertical",
            "",
            "Cresce rapidamente no inicio e desacelera",
            "Inverso da funcao exponencial!",
            "",
            "Pressione ENTER para iniciar!"
        },
        10
...
static TutorialScreen tutorial_rational[] = {
    {
        "FUNCAO RACIONAL",
        {
            ...
            "",
            "a, b: Parametros do numerador",
            "c, d: Parametros do denominador",
            "",
            "Funcao com comportamento interessante perto das assintotas!",
            "",
            "Pressione ENTER para iniciar!"
        },
        11
...
static TutorialScreen tutorial_trigonometric[] = {
    {
        "FUNCAO TRIGONOMETRICA",
        {
            ...
            "",
            "Cria ondas! Perfeito para trajetorias complexas.",
            "",
            "Pressione ENTER para iniciar!"
        },
        10
...
void draw_tutorial_screen_with_game(const TutorialScreen* screen, ALLEGRO_FONT* font, int page, int total_pages, Game* game) {
    ...
    // Título (giz amarelo claro) - MAIOR!
    draw_text_scaled(font, al_map_rgba(0, 0, 0, 100), SCREEN_WIDTH / 2 + 3, 83,
        ALLEGRO_ALIGN_CENTER, 2.5, screen->title);  // Sombra
    draw_text_scaled(font, al_map_rgb(255, 255, 200), SCREEN_WIDTH / 2, 80,
        ALLEGRO_ALIGN_CENTER, 2.5, screen->title);
    
    // Linha divisória sob o título (giz branco)
    al_draw_line(100, 130, SCREEN_WIDTH - 100, 130, al_map_rgb(220, 220, 220), 2);
    
    // Conteúdo (giz branco) - FONTE MAIOR!
    int y_pos = 170;
    for (int i = 0; i < screen->line_count; i++) {
        if (strlen(screen->content[i]) > 0) {
            // Sombra leve
            draw_text_scaled(font, al_map_rgba(0, 0, 0, 60), 122, y_pos + 2, 0, 1.8, screen->content[i]);
            // Texto principal
            draw_text_scaled(font, al_map_rgb(240, 240, 240), 120, y_pos, 0, 1.8, screen->content[i]);
        }
        y_pos += 40;  // Espaçamento aumentado para texto maior
    }
 ...
 void level_init_chapters(Chapter* chapters) {
    ...
    // FASE 3 - DESAFIO! Esquerda, centro E direita
    chapters[0].phases[2].arrows_allowed = 7;
    chapters[0].phases[2].epsilon_px = 18.0;
    chapters[0].phases[2].color_tint = TINT_GOLD;
    chapters[0].phases[2].collectible_count = 7;
    chapters[0].phases[2].items[0] = (Collectible){ -14.0, 1.8, 0.40, COLLECT_KEY, false };  // Extrema ESQUERDA
    chapters[0].phases[2].items[1] = (Collectible){ -4.0, 2.5, 0.40, COLLECT_KEY, false };   // Esquerda
    chapters[0].phases[2].items[2] = (Collectible){ 6.0, 3.2, 0.40, COLLECT_KEY, false };    // Centro-direita
    chapters[0].phases[2].items[3] = (Collectible){ -9.0, 2.0, 0.35, COLLECT_APPLE, false }; // Esquerda
    chapters[0].phases[2].items[4] = (Collectible){ 0.0, 2.8, 0.35, COLLECT_APPLE, false };  // Origem!
    chapters[0].phases[2].items[5] = (Collectible){ 11.0, 3.5, 0.35, COLLECT_APPLE, false }; // Direita
    chapters[0].phases[2].items[6] = (Collectible){ 17.0, 5.0, 0.40, COLLECT_APPLE_GOLD, false };  // Extrema direita
 
    // Obstáculos e inimigos - REDISTRIBUÍDOS
    chapters[0].test.obstacle_count = 1;
    chapters[0].test.obstacles[0].x = 12.0;  // Centro, não bloqueia chaves
    chapters[0].test.obstacles[0].y = 3.0;
    chapters[0].test.obstacles[0].width = 1.0;
    chapters[0].test.obstacles[0].height = 1.0;
    chapters[0].test.obstacles[0].type = OBS_ROCK;
    chapters[0].test.obstacles[0].has_physics = true;
    chapters[0].test.enemy_count = 0;
    chapters[0].test.has_boss = false;
    
    // FASE 1 - obstáculos espalhados + 1 inimigo
    chapters[0].phases[0].obstacle_count = 2;
    chapters[0].phases[0].obstacles[0].x = 7.5;  // Esquerda
    chapters[0].phases[0].obstacles[0].y = 1.2;
    chapters[0].phases[0].obstacles[0].width = 1.2;
    chapters[0].phases[0].obstacles[0].height = 1.2;
    chapters[0].phases[0].obstacles[0].type = OBS_ROCK;
    chapters[0].phases[0].obstacles[0].has_physics = true;
    
    chapters[0].phases[0].obstacles[1].x = 13.0;  // Direita
    chapters[0].phases[0].obstacles[1].y = 3.2;
    chapters[0].phases[0].obstacles[1].width = 0.8;
    chapters[0].phases[0].obstacles[1].height = 0.6;
    chapters[0].phases[0].obstacles[1].type = OBS_SPIKE;
    chapters[0].phases[0].obstacles[1].has_physics = false;
    
    chapters[0].phases[0].enemy_count = 1;
    chapters[0].phases[0].enemies[0].x = 11.0;  // Centro
    chapters[0].phases[0].enemies[0].y = 2.2;
    chapters[0].phases[0].enemies[0].move_speed = 2.0;
    chapters[0].phases[0].enemies[0].move_range = 3.5;
    chapters[0].phases[0].enemies[0].move_dir = 1.0;
    chapters[0].phases[0].enemies[0].start_x = 11.0;
    chapters[0].phases[0].enemies[0].active = true;
    chapters[0].phases[0].enemies[0].sprite = NULL;
    chapters[0].phases[0].has_boss = false;
    
    // FASE 2 - obstáculos espalhados + 2 inimigos
    chapters[0].phases[1].obstacle_count = 3;
    chapters[0].phases[1].obstacles[0].x = 6.5;  // Esquerda
    chapters[0].phases[1].obstacles[0].y = 1.0;
    chapters[0].phases[1].obstacles[0].width = 1.0;
    chapters[0].phases[1].obstacles[0].height = 1.0;
    chapters[0].phases[1].obstacles[0].type = OBS_ROCK;
    chapters[0].phases[1].obstacles[0].has_physics = true;
    
    chapters[0].phases[1].obstacles[1].x = 11.5;  // Centro
    chapters[0].phases[1].obstacles[1].y = 2.6;
    chapters[0].phases[1].obstacles[1].width = 1.0;
    chapters[0].phases[1].obstacles[1].height = 1.0;
    chapters[0].phases[1].obstacles[1].type = OBS_WALL;
    chapters[0].phases[1].obstacles[1].has_physics = true;
    
    chapters[0].phases[1].obstacles[2].x = 15.0;  // Direita
    chapters[0].phases[1].obstacles[2].y = 3.8;
    chapters[0].phases[1].obstacles[2].width = 0.8;
    chapters[0].phases[1].obstacles[2].height = 0.6;
    chapters[0].phases[1].obstacles[2].type = OBS_SPIKE;
    chapters[0].phases[1].obstacles[2].has_physics = false;
    
    chapters[0].phases[1].enemy_count = 2;
    chapters[0].phases[1].enemies[0].x = 8.5;  // Esquerda
    chapters[0].phases[1].enemies[0].y = 1.5;
    chapters[0].phases[1].enemies[0].move_speed = 2.5;
    chapters[0].phases[1].enemies[0].move_range = 2.5;
    chapters[0].phases[1].enemies[0].move_dir = 1.0;
    chapters[0].phases[1].enemies[0].start_x = 8.5;
    chapters[0].phases[1].enemies[0].active = true;
    chapters[0].phases[1].enemies[0].sprite = NULL;
    
    chapters[0].phases[1].enemies[1].x = 14.0;  // Direita
    chapters[0].phases[1].enemies[1].y = 3.5;
    chapters[0].phases[1].enemies[1].move_speed = 2.0;
    chapters[0].phases[1].enemies[1].move_range = 3.0;
    chapters[0].phases[1].enemies[1].move_dir = -1.0;
    chapters[0].phases[1].enemies[1].start_x = 14.0;
    chapters[0].phases[1].enemies[1].active = true;
    chapters[0].phases[1].enemies[1].sprite = NULL;
    chapters[0].phases[1].has_boss = false;
    
    // FASE 3 - obstáculos espalhados estrategicamente + 2 inimigos
    chapters[0].phases[2].obstacle_count = 3;
    chapters[0].phases[2].obstacles[0].x = 6.0;  // Esquerda baixa
    chapters[0].phases[2].obstacles[0].y = 0.9;
    chapters[0].phases[2].obstacles[0].width = 1.0;
    chapters[0].phases[2].obstacles[0].height = 1.0;
    chapters[0].phases[2].obstacles[0].type = OBS_ROCK;
    chapters[0].phases[2].obstacles[0].has_physics = true;
    
    chapters[0].phases[2].obstacles[1].x = 11.0;  // Centro médio
    chapters[0].phases[2].obstacles[1].y = 2.4;
    chapters[0].phases[2].obstacles[1].width = 1.0;
    chapters[0].phases[2].obstacles[1].height = 1.0;
    chapters[0].phases[2].obstacles[1].type = OBS_WALL;
    chapters[0].phases[2].obstacles[1].has_physics = true;
    
    chapters[0].phases[2].obstacles[2].x = 14.5;  // Direita alta
    chapters[0].phases[2].obstacles[2].y = 3.9;
    chapters[0].phases[2].obstacles[2].width = 0.8;
    chapters[0].phases[2].obstacles[2].height = 0.6;
    chapters[0].phases[2].obstacles[2].type = OBS_SPIKE;
    chapters[0].phases[2].obstacles[2].has_physics = false;
    
    chapters[0].phases[2].enemy_count = 2;
    chapters[0].phases[2].enemies[0].x = 8.0;  // Esquerda
    chapters[0].phases[2].enemies[0].y = 1.2;
    chapters[0].phases[2].enemies[0].move_speed = 2.5;
    chapters[0].phases[2].enemies[0].move_range = 2.5;
    chapters[0].phases[2].enemies[0].move_dir = 1.0;
    chapters[0].phases[2].enemies[0].start_x = 8.0;
    chapters[0].phases[2].enemies[0].active = true;
    chapters[0].phases[2].enemies[0].sprite = NULL;
    
    chapters[0].phases[2].enemies[1].x = 13.5;  // Direita
    chapters[0].phases[2].enemies[1].y = 3.2;
    chapters[0].phases[2].enemies[1].move_speed = 3.0;
    chapters[0].phases[2].enemies[1].move_range = 3.0;
    chapters[0].phases[2].enemies[1].move_dir = -1.0;
    chapters[0].phases[2].enemies[1].start_x = 13.5;
    chapters[0].phases[2].enemies[1].active = true;
    chapters[0].phases[2].enemies[1].sprite = NULL;
    chapters[0].phases[2].has_boss = false;
    
    // Boss phase do capítulo 0
    chapters[0].boss_phase.arrows_allowed = 15;  // Mais flechas para o boss
    ...
    chapters[0].boss_phase.boss.x = 10.0;
    chapters[0].boss_phase.boss.y = 2.0;
    chapters[0].boss_phase.boss.health = 50.0;         // REDUZIDO! Era 100.0 (agora 5 acertos)
    chapters[0].boss_phase.boss.max_health = 50.0;     // Mais fácil!
    chapters[0].boss_phase.boss.shoot_timer = 0.0;
    chapters[0].boss_phase.boss.shoot_cooldown = 3.0;  // AUMENTADO: atira MENOS (era 1.8s)
    chapters[0].boss_phase.boss.move_speed = 2.5;      // REDUZIDO: move mais devagar (era 3.5)
    chapters[0].boss_phase.boss.move_range = 4.0;      // REDUZIDO: patrulha menor (era 5.0)
    chapters[0].boss_phase.boss.start_x = 10.0;
    chapters[0].boss_phase.boss.move_dir = 1.0;
    chapters[0].boss_phase.boss.active = true;
    chapters[0].boss_phase.boss.sprite = NULL;
 ...
 void game_update(Game* game, double dt) {
     ...
     case STATE_PLAY:
        ...
        if (game->current_function.type == FN_LINEAR) {
         ...
         if (game->key_e) {
            ...
            // RACIONAL e TRIGONOMÉTRICA: M→A, H→B, K→C, mais tecla especial para D
            if (game->key_m) { game->ui.editor.selected = PARAM_A; game->key_m = false; }
            if (game->key_h) { game->ui.editor.selected = PARAM_H; game->key_h = false; }
            if (game->key_k) { game->ui.editor.selected = PARAM_KQ; game->key_k = false; }
            // Usar tecla especial para D (por exemplo, tecla D mesmo, mas conflita com movimentação)
            // Por ora, vamos usar bracket [ ] para D
    ...
    case STATE_BOSS_FIGHT:
        ...
        if (game->arrow.active) {
            ...
            // Colisão com o boss
            check_arrow_boss_collision(&game->arrow, boss);
        }
        ...
        // Derrota: sem flechas e boss ainda vivo, OU player morreu
        if ((game->arrows_remaining <= 0 && !game->arrow.active && boss->active) || game->player.health <= 0) {
             game->state = STATE_TRY_AGAIN;
         }
         break;
     ...
     case STATE_STAGE_CLEAR:
         if (game->key_enter) {
            ...
            if (game->current_phase == 0) {
                ...
                // Completou fase 1 ou 2, vai para próxima
                 game_start_phase(game, game->current_phase + 1);
             }
            else if (game->current_phase == 3) {
                // Completou fase 3, inicia BOSS FIGHT!
...
void draw_command_hud(Game* game, ALLEGRO_FONT* font) {
    // Background semi-transparente sempre visível
    int hud_y = SCREEN_HEIGHT - 60;
    al_draw_filled_rectangle(0, hud_y, SCREEN_WIDTH, SCREEN_HEIGHT, 
        al_map_rgba(0, 0, 0, 220));
    
    // Borda superior
    al_draw_line(0, hud_y, SCREEN_WIDTH, hud_y, al_map_rgb(100, 100, 100), 2);
    
    // Comandos específicos por tipo de função
    const char* commands = NULL;
    const char* function_info = NULL;
    
    switch (game->current_function.type) {
    case FN_LINEAR:
        commands = "[<-/->]mover [ESPACO]atirar [M]m [K]k [Q/E]ajustar [C]trocar [H]especial [R]reiniciar";
        function_info = "LINEAR: y = mx + k";
        break;
    case FN_QUADRATIC:
        commands = "[<-/->]mover [ESPACO]atirar [M]a [H]h [K]k [Q/E]ajustar [C]trocar [H]especial";
        function_info = "QUADRATICA: y = a(x-h)^2 + k";
        break;
    case FN_EXPONENTIAL:
        commands = "[<-/->]mover [ESPACO]atirar [M]a [H]b [K]c [Q/E]ajustar [C]trocar [H]especial";
        function_info = "EXPONENCIAL: y = a*b^x + c";
        break;
    case FN_LOGARITHMIC:
        commands = "[<-/->]mover [ESPACO]atirar [M]a [H]b [K]c [Q/E]ajustar [C]trocar [H]especial";
        function_info = "LOGARITMICA: y = a*log(x+b) + c";
        break;
    case FN_RATIONAL:
        commands = "[<-/->]mover [ESPACO]atirar [M]a [H]b [K]c [D]d [Q/E]ajustar [C]trocar";
        function_info = "RACIONAL: y = (ax+b)/(cx+d)";
        break;
    case FN_TRIGONOMETRIC:
        commands = "[<-/->]mover [ESPACO]atirar [M]a [H]b [K]c [D]d [Q/E]ajustar [C]trocar";
        function_info = "TRIGONOMETRICA: y = a*sin(bx+c) + d";
        break;
    }
    
    // Desenhar nome da função (lado esquerdo)
    if (function_info) {
        al_draw_text(font, al_map_rgb(200, 255, 200), 10, hud_y + 8, 0, function_info);
    }
    
    // Desenhar comandos (centralizado)
    if (commands) {
        al_draw_text(font, al_map_rgb(255, 255, 255), SCREEN_WIDTH / 2, hud_y + 25,
            ALLEGRO_ALIGN_CENTER, commands);
    }
    ...
    if (game->special_active) {
        ...
        // Banner de especial ativo (lado direito)
        int banner_x = SCREEN_WIDTH - 250;
        int banner_y = hud_y - 60;
        al_draw_filled_rounded_rectangle(banner_x, banner_y, SCREEN_WIDTH - 10, banner_y + 45,
            8, 8, al_map_rgba(0, 0, 0, 200));
        al_draw_text(font, special_color, banner_x + 120, banner_y + 5,
            ALLEGRO_ALIGN_CENTER, special_name);
        
        char timer_text[32];
        snprintf(timer_text, sizeof(timer_text), "Tempo: %.1fs", game->special_timer);
        al_draw_text(font, al_map_rgb(200, 200, 200), banner_x + 120, banner_y + 25,
            ALLEGRO_ALIGN_CENTER, timer_text);
    }
 }
 ...
 void game_draw(Game* game) {
     ...
     switch (game->state) {
    case STATE_TITLE: {
        // Background melhorado - imagem ou gradiente bonito
        if (game->title_background) {
            al_draw_scaled_bitmap(game->title_background, 0, 0,
                al_get_bitmap_width(game->title_background),
                al_get_bitmap_height(game->title_background),
                0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0);
        } else {
            // Gradiente azul escuro → roxo escuro
            for (int i = 0; i < SCREEN_HEIGHT; i++) {
                float ratio = (float)i / SCREEN_HEIGHT;
                ALLEGRO_COLOR line_color = al_map_rgb(
                    (int)(20 + 60 * ratio),   // 20 → 80 (R)
                    (int)(20 + 20 * ratio),   // 20 → 40 (G)
                    (int)(80 - 20 * ratio)    // 80 → 60 (B)
                );
                al_draw_line(0, i, SCREEN_WIDTH, i, line_color, 1);
            }
        }
        ...
        // Título ARCHER com sombra e escala grande
        // Sombra
        al_draw_text(font, al_map_rgba(0, 0, 0, 180), SCREEN_WIDTH / 2 + 4, 154,
             ALLEGRO_ALIGN_CENTER, "ARCHER");
        al_draw_text(font, al_map_rgba(0, 0, 0, 180), SCREEN_WIDTH / 2 + 4, 154,
            ALLEGRO_ALIGN_CENTER, "ARCHER");
        // Título principal (com gradiente simulado)
        al_draw_text(font, al_map_rgb(255, 255, 150), SCREEN_WIDTH / 2, 150,
            ALLEGRO_ALIGN_CENTER, "ARCHER");
        al_draw_text(font, al_map_rgb(255, 200, 50), SCREEN_WIDTH / 2, 152,
            ALLEGRO_ALIGN_CENTER, "ARCHER");
        
        // Subtítulo com sombra
        al_draw_text(font, al_map_rgba(0, 0, 0, 120), SCREEN_WIDTH / 2 + 2, 282,
             ALLEGRO_ALIGN_CENTER, "Jogo de Funcoes Matematicas");
        al_draw_text(font, al_map_rgb(200, 220, 255), SCREEN_WIDTH / 2, 280,
            ALLEGRO_ALIGN_CENTER, "Jogo de Funcoes Matematicas");
     ...
     case STATE_PLAY: {
        ...
        for (int i = 0; i < phase->obstacle_count; i++) {
            ...
            case OBS_WALL:
                ...
                break;
            case OBS_SPIKE:
                obs_color = al_map_rgb(150, 150, 150);
                break;
            }
            ...
            if (obs_sprite && obs->type == OBS_ROCK) {
                ...
                if (obs->type == OBS_SPIKE) {
                    // Triângulo de espinho
                    al_draw_filled_triangle(ox, oy - obs_h, 
                        ox - obs_w/2, oy, 
                        ox + obs_w/2, oy, obs_color);
                    al_draw_triangle(ox, oy - obs_h, 
                        ox - obs_w/2, oy, 
                        ox + obs_w/2, oy, 
                        al_map_rgb(255, 255, 255), 2);
                } else {
                    // Retângulo para rock/wall
        ...
        // Coletáveis - com sprites BEM MAIORES!
         for (int i = 0; i < phase->collectible_count; i++) {
            ...
            int sprite_size = (int)(c->radius * PIXELS_PER_UNIT * 5.0);  // 5x maior!
    ...
    case STATE_STAGE_CLEAR: {
        ...
        // Tela especial se completou o BOSS!
        if (game->current_phase == 4) {
