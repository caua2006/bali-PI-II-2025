...
// PROTÓTIPOS DE FUNÇoESS

void draw_text_scaled(ALLEGRO_FONT* font, ALLEGRO_COLOR color, float x, float y, int flags, float scale, const char* text);
bool check_position_collision(double x, double y, double radius, Collectible* collectibles, int collectible_count,
...
void draw_grid_frame(ALLEGRO_BITMAP* sheet, int frame, int cols, int rows,int dest_x, int dest_y, int dest_w, int dest_h, int flags) {
...
void arrow_draw_preview(double x0, double y0, double vx, const MathFunction* fn, ALLEGRO_COLOR color, const Camera* cam) {
...
void level_init_chapters(Chapter* chapters) {
    ...
    chapters[0].phases[0].items[0] = (Collectible){ -10.0, 1.2, 0.40, COLLECT_KEY, false };  
    chapters[0].phases[0].items[1] = (Collectible){ 0.0, 2.0, 0.40, COLLECT_KEY, false };    
    chapters[0].phases[0].items[2] = (Collectible){ 10.0, 2.8, 0.40, COLLECT_KEY, false };   
    chapters[0].phases[0].items[3] = (Collectible){ -6.0, 1.5, 0.35, COLLECT_APPLE, false }; 
    chapters[0].phases[0].items[4] = (Collectible){ 5.0, 2.2, 0.35, COLLECT_APPLE, false };  
    chapters[0].phases[0].items[5] = (Collectible){ 14.0, 3.8, 0.40, COLLECT_APPLE_GOLD, false };  
    ...
    chapters[0].phases[1].items[0] = (Collectible){ -12.0, 1.5, 0.40, COLLECT_KEY, false };  
    chapters[0].phases[1].items[1] = (Collectible){ -3.0, 2.2, 0.40, COLLECT_KEY, false };   
    chapters[0].phases[1].items[2] = (Collectible){ 8.0, 2.8, 0.40, COLLECT_KEY, false };    
    chapters[0].phases[1].items[3] = (Collectible){ -7.0, 1.8, 0.35, COLLECT_APPLE, false }; 
    chapters[0].phases[1].items[4] = (Collectible){ 4.0, 2.4, 0.35, COLLECT_APPLE, false };  
    chapters[0].phases[1].items[5] = (Collectible){ 15.0, 4.0, 0.40, COLLECT_APPLE_GOLD, false };  

    // Obstáculos 
    chapters[0].test.obstacle_count = 2;  
    chapters[0].test.obstacles[0].x = -4.0;
    chapters[0].test.obstacles[0].y = 1.8;
    chapters[0].test.obstacles[0].width = 1.5;  
    chapters[0].test.obstacles[0].height = 1.5; 
    ...
    chapters[0].phases[0].obstacle_count = 3;  
...
bool check_position_collision(double x, double y, double radius,Collectible* collectibles, int collectible_count,Obstacle* obstacles, int obstacle_count, Enemy* enemies, int enemy_count,int skip_collectible, int skip_obstacle, int skip_enemy) {
...
void draw_command_hud(Game* game, ALLEGRO_FONT* font) {
    ...
    if (game->special_active) {
        ...
        // Banner de especial ativo 
        int banner_x = SCREEN_WIDTH - 250;
        int banner_y = 80 + 400 + 20;  
