...
static const char* ASSET_BG_LOGARITMICA = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/bg/logaritmica.jpg";
static const char* ASSET_BG_RACIONAL = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/bg/racional.jpg";
static const char* ASSET_BG_TRIGONOMETRICA = "C:/Users/renan/source/repos/BALI-PI/x64/Debug/assets/bg/trigonometrica.jpg";
...
void arrow_shoot(Arrow* arrow, double x0, double y0, double vx, double charge_power) {
 ...
 void level_init_chapters(Chapter* chapters) {
    ...
    chapters[0].test.obstacles[0] = (Obstacle){ 5.0, -7.0, 1.0, 1.5, OBS_ROCK };
    chapters[0].test.obstacles[1] = (Obstacle){ 12.5, -6.5, 0.8, 0.8, OBS_ENEMY };
    chapters[0].test.obstacles[2] = (Obstacle){ 18.0, -7.5, 1.2, 0.6, OBS_SPIKE };
    ...
    chapters[0].phases[0].obstacles[0] = (Obstacle){ 6.0, -7.5, 1.0, 1.0, OBS_ROCK };
    chapters[0].phases[0].obstacles[1] = (Obstacle){ 13.0, -6.0, 0.7, 0.7, OBS_ENEMY };
    ...
    chapters[0].phases[1].obstacles[0] = (Obstacle){ 7.0, -8.0, 0.9, 1.2, OBS_ROCK };
    chapters[0].phases[1].obstacles[1] = (Obstacle){ 11.0, -7.0, 0.8, 0.8, OBS_ENEMY };
    chapters[0].phases[1].obstacles[2] = (Obstacle){ 15.5, -6.5, 1.0, 0.5, OBS_SPIKE };
    ...
    chapters[0].phases[2].obstacles[0] = (Obstacle){ 6.5, -8.0, 1.0, 1.3, OBS_ROCK };
    chapters[0].phases[2].obstacles[1] = (Obstacle){ 10.5, -7.0, 0.8, 0.8, OBS_ENEMY };
    chapters[0].phases[2].obstacles[2] = (Obstacle){ 14.0, -6.5, 0.7, 0.7, OBS_ENEMY };
    chapters[0].phases[2].obstacles[3] = (Obstacle){ 17.5, -7.5, 1.1, 0.6, OBS_SPIKE };
 ...
 void game_draw(Game* game) {
    ...
    case STATE_PLAY: {
        ...
        // ========== PLANO CARTESIANO XY ==========
        // Desenhar grade de coordenadas para ajudar o jogador!
        {
            ALLEGRO_COLOR grid_color = al_map_rgba(200, 200, 200, 80);   // Linhas fracas
            ALLEGRO_COLOR axis_color = al_map_rgba(255, 255, 255, 150);  // Eixos principais
            ALLEGRO_COLOR number_color = al_map_rgba(200, 200, 200, 200);
            
            // Desenhar linhas verticais (grade X)
            for (int x = -20; x <= 20; x++) {
                int sx, sy_top, sy_bottom;
                world_to_screen(&game->camera, x, 15.0, &sx, &sy_top);
                world_to_screen(&game->camera, x, -15.0, &sx, &sy_bottom);
                
                if (x == 0) {
                    // Eixo Y (x=0) mais forte
                    al_draw_line(sx, sy_top, sx, sy_bottom, axis_color, 2);
                } else {
                    // Linhas de grade normais
                    al_draw_line(sx, sy_top, sx, sy_bottom, grid_color, 1);
                }
                
                // Números no eixo X (a cada 2 unidades para não poluir)
                if (x % 2 == 0 && x != 0) {
                    char num[8];
                    snprintf(num, sizeof(num), "%d", x);
                    int text_x, text_y;
                    world_to_screen(&game->camera, x, -0.5, &text_x, &text_y);
                    al_draw_text(font, number_color, text_x, text_y, 
                        ALLEGRO_ALIGN_CENTER, num);
                }
            }
            
            // Desenhar linhas horizontais (grade Y)
            for (int y = -15; y <= 15; y++) {
                ...
                if (y == 0) {
                    // Eixo X (y=0) mais forte
                    al_draw_line(sx_left, sy, sx_right, sy, axis_color, 2);
                } else {
                    // Linhas de grade normais
                    al_draw_line(sx_left, sy, sx_right, sy, grid_color, 1);
                }
                
                // Números no eixo Y (a cada 2 unidades)
                if (y % 2 == 0 && y != 0) {
                    char num[8];
                    snprintf(num, sizeof(num), "%d", y);
                    int text_x, text_y;
                    world_to_screen(&game->camera, -0.8, y, &text_x, &text_y);
                    al_draw_text(font, number_color, text_x, text_y, 
            ...
            al_draw_filled_circle(ox, oy, 4, al_map_rgba(255, 255, 255, 180));
            al_draw_text(font, al_map_rgb(255, 255, 255), ox + 10, oy - 10, 0, "O(0,0)");
        }
        ...
        if (game->show_max_hint) {
            ...
            al_draw_text(font, al_map_rgb(150, 150, 150), SCREEN_WIDTH / 2, panel_y + 220,
                ALLEGRO_ALIGN_CENTER, "Ajuste os parametros para acertar os alvos!");
        }
